...
<template><PageHero title="陈列展览" description="展示基本陈列、专题展览和临时展览的内容结构。" :image="images.exhibitionHall" /><section class="section"><div class="shell"><p v-if="loading" class="source-note">资料加载中…</p><p v-else-if="usingFallback" class="source-note">当前展示本地资料。</p><div v-if="displayExhibitions.length" class="card-grid"><article v-for="item in displayExhibitions" :key="item.id" class="content-card"><img :src="item.image" :alt="item.title" /><div class="content-card__body"><span class="tag">{{ item.type }}</span><h3>{{ item.title }}</h3><p v-if="item.time" class="card-meta">{{ item.time }}</p><p>{{ item.summary }}</p></div></article></div><div v-else class="empty-state">暂无展览资料。</div></div></section></template>
