...
test('home timeline requests only featured verified history records from the API', () => {
  assert.match(source, /fetchFeaturedHistoryEvents\(\{ page: 1, pageSize: 7 \}\)/)
  assert.match(source, /v-for="\(item, index\) in historyHighlights"/)
  assert.match(source, /item\.timeText/)
  assert.doesNotMatch(source, /homeNarrative\.timeline/)
})
